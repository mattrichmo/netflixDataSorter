...
const processItem = async (item, paths) => {
    try {
        ...
        // Update to fetch information using the new item structure
        ...
        fs.appendFileSync(paths.outputFilePath, `${JSON.stringify(result)}\n`);

        await manifestWrite({
            status: result.status,
            meta: { // Ensure this structure matches how you're expecting to read it in manifestWrite
                indexNum: item.meta.indexNum, // Correctly place indexNum inside meta
                coreTitle: item.meta.coreTitle
            },
            imdbLink: result.data ? result.data.imdbLink : null,
